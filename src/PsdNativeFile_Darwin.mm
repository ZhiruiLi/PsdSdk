...
{
    const dispatch_fd_t fileDescriptor = static_cast<dispatch_fd_t>(m_file);
    std::wstring_convert<std::codecvt_utf8<wchar_t>,wchar_t> convert;
    std::string s = convert.to_bytes(filename);
    char const *cs = s.c_str();
    fileDescriptor = open(cs, O_RDONLY);
    if (fileDescriptor == -1)
    {
...
bool NativeFile::DoOpenWrite(const wchar_t* filename)
{
    ...
    std::string s = convert.to_bytes(filename);
    char const *cs = s.c_str();
    fileDescriptor = open(cs, O_WRONLY|O_CREAT|O_TRUNC, S_IRUSR|S_IWUSR|S_IRGRP);
    if (fileDescriptor == -1)
    {
        PSD_ERROR("NativeFile", "Cannot obtain handle for file \"%ls\".", filename);
        return false;
...
bool NativeFile::DoClose(void)
{
    ...
    const int success = close(fileDescriptor);
    if (success == -1)
    {
...
bool NativeFile::DoWaitForRead(File::ReadOperation& operation)
{
    ...
    lseek(fileDescriptor, op->offset, SEEK_SET);
    op->semaphore = dispatch_semaphore_create(0);
    dispatch_read(fileDescriptor, op->length, queue, op->ioHandler);
...
bool NativeFile::DoWaitForWrite(File::WriteOperation& operation)
{
    ...
    lseek(fileDescriptor, op->offset, SEEK_SET);
    op->semaphore = dispatch_semaphore_create(0);
    dispatch_write(fileDescriptor, op->dataToWrite, queue, op->ioHandler);
